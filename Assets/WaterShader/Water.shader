	...
	SubShader{
		...
		CGPROGRAM
			...
			void disp(inout appdata v) {
				...
				waveTex1 = tex2Dlod(_WaveTex1H, float4(v.texcoord1.xy * _WaveTiling.x + float2(0, _Time.x * _FlowSpeed.x), 0, 0)).a * _Displace.x;
				waveTex2 = tex2Dlod(_WaveTex2H, float4(v.texcoord1.xy * _WaveTiling.y + float2(0, _Time.x * _FlowSpeed.y), 0, 0)).a * _Displace.y;
				fixed displaceB = waveTex1 + waveTex2;

				fixed displaceBlend = lerp(displace, displaceB, v.color.r);

				v.vertex.xyz += v.normal * displaceBlend;
			}
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				waveTex1 = tex2D(_WaveTex1, IN.uv2_WaveTex2 * _WaveTiling.x + float2(0, _Time.x * _FlowSpeed.x));
				waveTex2 = tex2D(_WaveTex2, IN.uv2_WaveTex2 * _WaveTiling.y + float2(0, _Time.x * _FlowSpeed.x));

				normal1 = UnpackNormal(waveTex1);
				normal2 = UnpackNormal(waveTex2);
				fixed3 normalB = BlendNormals(normal1, normal2);

				distortion1 = UnpackScaleNormal(waveTex1, _Refraction.x);
				distortion2 = UnpackScaleNormal(waveTex2, _Refraction.y);
				fixed2 distortionB = BlendNormals(distortion1, distortion2).rg;


				fixed3 normalBlend = lerp(normal, normalB, IN.color);
				fixed2 distortionBlend = lerp(distortion, distortionB, IN.color);


				half2 grabUV = (IN.screenPos.xy / IN.screenPos.w) * float2(1, -1) + float2(0, 1);
				half3 grab = tex2D(_GrabTexture, grabUV + distortionBlend).rgb * _Color;
